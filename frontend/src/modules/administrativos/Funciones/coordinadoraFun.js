export async function obtenerLevantamientos(tipo) {
  try {
  const res = await fetch("https://backend-qa-one.vercel.app/formularios/levantamientosF");
  ...
  const seguimientoRes = await fetch(`https://backend-qa-one.vercel.app/formularios/seguimiento/${solicitudes[i].idformulario}`);
      const seguimientoData = await seguimientoRes.json();

  const estadoRes = await fetch(`https://backend-qa-one.vercel.app/formularios/estado/${seguimientoData[0].idestado}`);
      const estadoData = await estadoRes.json();

      solicitudes[i].estado = estadoData[0].nombre;

  const sedeRes = await fetch(`https://backend-qa-one.vercel.app/formularios/sede/${solicitudes[i].sede}`);
...
export async function actualizarEstado(idformulario, idestado) {
  try {
  const res = await fetch('https://backend-qa-one.vercel.app/coordinadora/actualizarEstado', {
      method: 'PUT',
      headers: {
...
export const obtenerRequisitosAutomaticos = async () => {
  try {
  const response = await fetch('https://backend-qa-one.vercel.app/coordinadora/requerimientosLevAuto'); 
...
export const obtenerCursos = async () => {
  try {
  const response = await fetch('https://backend-qa-one.vercel.app/cursos/getIdcurso'); 
    const data = await response.json();
    return data;
  } catch (error) {
...
export const insertarRequisitoAutomatico = async (requisito) => {
  try {
  const response = await fetch('https://backend-qa-one.vercel.app/coordinadora/insertarLenvAuto', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const eliminarRequisitoAutomatico = async (id) => {
  try {
  const response = await fetch(`https://backend-qa-one.vercel.app/coordinadora/deleRequiAuto/${id}`, {
      method: 'DELETE'
    });
...
export const obtenerRequisitoPorId = async (id) => {
  const response = await fetch(`https://backend-qa-one.vercel.app/coordinadora/getrequisitosAutomaticos/${id}`);
  if (!response.ok) throw new Error('No se pudo obtener el requisito');
  return await response.json();
};

export const actualizarRequisito = async (id, datos) => {
  const response = await fetch(`https://backend-qa-one.vercel.app/coordinadora/updateRequisitoAuto/${id}`, {
    method: 'PUT',
