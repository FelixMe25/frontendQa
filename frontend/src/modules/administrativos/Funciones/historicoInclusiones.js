...
export async function informacion(tipo) {
    try {
  const response = await fetch("https://backend-qa-one.vercel.app/formularios/inclusionesF");
        if (!response.ok) {
        throw new Error('Error en la respuesta de la API');
        }
        const data = await response.json();
        ...
        for (let i = 0; i < solicitudes.length; i++) {
            const response = await fetch(`https://backend-qa-one.vercel.app/formularios/seguimiento/${solicitudes[i].idformulario}`);
            if (!response.ok) {
                throw new Error('Error en la respuesta de la API');
            }
            ...
            const estado = await fetch(`https://backend-qa-one.vercel.app/formularios/estado/${estadodata[0].idestado}`);
            let estadoString = await estado.json();
            solicitudes[i].estado = estadoString[0].nombre;

            const sede = await fetch(`https://backend-qa-one.vercel.app/formularios/sede/${solicitudes[i].sede}`);
           ...
           solicitudes[i].sede = sedeString[0].nombre;
            ...
            if (solicitudes[i].beca === null) {
                ...
                const beca = await fetch(`https://backend-qa-one.vercel.app/formularios/tipobeca/${solicitudes[i].beca}`);
