...
export async function informacion(tipo) {
    try {
    const response = await fetch("https://backend-qa-one.vercel.app/usuarios");
        if (!response.ok) {
        ...
        for (let i = 0; i < solicitudes.length; i++) {
            ...
            const estado = await fetch(`https://backend-qa-one.vercel.app/usuarios/tipo/${solicitudes[i].tipo}`);
