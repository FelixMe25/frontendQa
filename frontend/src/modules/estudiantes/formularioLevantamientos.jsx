...
const FormularioLevantamiento = () => {
  ...
  useEffect(() => {
    const fetchUsuarioDetallado = async () => {
      ...
      try {
  const res = await fetch(`https://backend-qa-one.vercel.app/usuariodetallado/${usuario.idusuario}`);
    ...
    const fetchCursos = async () => {
      try {
  const res = await fetch('https://backend-qa-one.vercel.app/cursos');
        const data = await res.json();
        if (res.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
  const res = await fetch('https://backend-qa-one.vercel.app/formularios/levantamientos', {
        method: 'POST',
