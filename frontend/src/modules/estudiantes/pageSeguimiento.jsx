...
const PageSeguimiento = () => {
  ...
  useEffect(() => {
    const obtenerSeguimiento = async () => {
      ...
      try {
  const res = await fetch(`https://backend-qa-one.vercel.app/seguimientoUsuario/${usuario.idusuario}`);
  ...
  const handleEliminar = async (solicitud) => {
    ...
    try {
  const res = await fetch(`https://backend-qa-one.vercel.app/seguimientoUsuario/${solicitud.idformulario}`, { method: 'DELETE' });
      if (res.ok) {
        // Refresca la lista desde el backend para asegurar que está actualizada
  const res2 = await fetch(`https://backend-qa-one.vercel.app/seguimientoUsuario/${usuario.idusuario}`);
